...
export async function getMultiSearchResults(search, page) {
  const { data } = await axios.get(
    `${apiEndPoint}/search/multi?api_key=${apiKey}&language=en-US&query=${search}&page=${page}&include_adult=false`
  );
  return data;
}
